...
def output_content(fp, xls_head_, xls_row_grid, xls_col_grid, indent):
    for xls_row in xls_row_grid.row_list:
        ...
        if xls_row.get_cell(xls_head_.conf['inc']['col']).content:
            ...
            content = [indent, xls_row.get_cell(xls_head_.conf['key']['col']).content,
...
def generate_common_define_make_file(xls_head_, xls_row_grid, xls_col_grid, output_dir, install_prefix):
    file_path = os.path.join(output_dir, 'common_define.make')
...
def test_001():
    ...
    assert 0 == generate_common_define_make_file(xls_head_, xls_loader.row_grid, xls_loader.col_grid,
