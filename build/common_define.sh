...
FREE_TYPE_BASENAME=freetype-2.10.0
...
SRC_DIR_LIST=(${M4_SRC_DIR} ${AUTOCONF_SRC_DIR} ${AUTOMAKE_SRC_DIR} ${LIBTOOL_SRC_DIR} ${HELPTOMAN_SRC_DIR} ${BISON_SRC_DIR} ${FLEX_SRC_DIR} ${CMAKE_SRC_DIR} ${CURL_SRC_DIR} ${SCONS_SRC_DIR} ${BOOST_SRC_DIR} ${ZLIB_SRC_DIR} ${XZ_SRC_DIR} ${EXIV2_SRC_DIR} ${FLATBUF_SRC_DIR} ${FREE_TYPE_SRC_DIR} ${GFLAGS_SRC_DIR} ${GLOG_SRC_DIR} ${GTEST_SRC_DIR} ${GPERFTOOLS_SRC_DIR} ${HIREDIS_SRC_DIR} ${IMAGE_MAGICK_SRC_DIR} ${JEMALLOC_SRC_DIR} ${JSONCPP_SRC_DIR} ${LIBEVENT_SRC_DIR} ${LIBJPEG_SRC_DIR} ${LIBMAGIC_SRC_DIR} ${LIBPNG_SRC_DIR} ${LIBTIFF_SRC_DIR} ${LIBUNWIND_SRC_DIR} ${LIBUUID_SRC_DIR} ${LIBXML2_SRC_DIR} ${LOG4CPLUS_SRC_DIR} ${NCURSES_SRC_DIR} ${READLINE_SRC_DIR} ${LUA_SRC_DIR} ${LIBBSON_SRC_DIR} ${MONGO_C_DRIVER_SRC_DIR} ${MONGO_CXX_DRIVER_SRC_DIR} ${MSGPACK_SRC_DIR} ${OPENCC_SRC_DIR} ${OPENSSL_SRC_DIR} ${PCRE_SRC_DIR} ${PROTOBUF_SRC_DIR} ${RAPIDJSON_SRC_DIR} ${REDIS_SRC_DIR} ${SNAPPY_SRC_DIR} ${SWIG_SRC_DIR} ${NGINX_SRC_DIR} ${TENGINE_SRC_DIR} ${THRIFT_SRC_DIR} ${ZOOKEEPER_SRC_DIR} ${OPENPGM_SRC_DIR} ${ZEROMQ_SRC_DIR} ${UDT_SRC_DIR} ${MYSQL_CLIENT_SRC_DIR} ${LEVELDB_SRC_DIR} ${MUPDF_SRC_DIR} ${ASSIMP_SRC_DIR} ${FFMPEG_SRC_DIR} ${ASTYLE_SRC_DIR} ${SPDLOG_SRC_DIR} ${RTMPDUMP_SRC_DIR} ${SRS_SRC_DIR} ${FDK_AAC_SRC_DIR} ${X264_SRC_DIR} ${NASM_SRC_DIR} ${SDL_SRC_DIR} ${OPENH264_SRC_DIR} ${HTTP_PARSER_SRC_DIR})

LIB_LIST=(m4 autoconf automake libtool helptoman bison flex cmake curl scons boost zlib xz exiv2 flatbuf free_type gflags glog gtest gperftools hiredis image_magick jemalloc jsoncpp libevent libjpeg libmagic libpng libtiff libunwind libuuid libxml2 log4cplus ncurses readline lua libbson mongo_c_driver mongo_cxx_driver msgpack opencc openssl pcre protobuf rapidjson redis snappy swig nginx tengine thrift zookeeper openpgm zeromq udt mysql_client leveldb mupdf assimp ffmpeg astyle spdlog rtmpdump srs fdk_aac x264 nasm sdl openh264 http_parser)
...
function UnzipAll()
{
    ...
    tar -C ${ARCHIVES_DIR} -xvf ${ARCHIVES_DIR}/${FREE_TYPE_BASENAME}.tar.bz2
    if [ $? -ne 0 ]; then
        echo "failed to unzip" ${ARCHIVES_DIR}/${FREE_TYPE_BASENAME}.tar.bz2
        exit 1
    fi
